 ...
 * Initial condition: a spherical cap drop of heigh h0 (= 1) and base radius Gamma!.
 * Details of the spherical cap initial condition: [https://en.wikipedia.org/wiki/Spherical_cap](https://en.wikipedia.org/wiki/Spherical_cap)
...
double Oh1, Oh2, Bo, Gamma, Ldomain, DeltaMin, theta0 = 10*pi/180.0, thetaA = 60.0*pi/180.0, thetaR = 30.0*pi/180.0;
int MAXlevel;
...
int main(int argc, char const *argv[]) {
  ...
  Oh2 = Oh1*MuR;
  Bo = 1e-3;
  Gamma = sin(theta0)/(1-cos(theta0)); // radius of the spherical cap!
...
event init(t = 0){
  if(!restore (file = "dump")){
    ...
    double R0 = 1e0/(1-cos(theta0));
    /**
    We can now initialize the volume fractions in the domain. */
    refine(x < h0 + 10*DeltaMin && level<MAXlevel);

    fraction(f, sq(R0)-sq(x+(R0-1e0))-sq(y));
