...
int main(int a, char const *arguments[])
{
  ...
  list = list_add (list, momx);
  list = list_add (list, momy);
  ...
  */
  ...
  foreach() {
    ...
    momx[] =(clamp(f[], 0., 1.) * rho1 + clamp((1.-f[]), 0., 1.) * rho2)*u.x[];
    momy[] =(clamp(f[], 0., 1.) * rho1 + clamp((1.-f[]), 0., 1.) * rho2)*u.y[];
  }
